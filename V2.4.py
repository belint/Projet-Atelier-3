...
def mosaique(chemin_im_source):
	...
	Moyenne_Image_Source(im_source_RGB,im_source_HSV,new_size,tab_histo,tab,tab_non_convert,pix_copy)
...
def Best_Comp(tab_histo,histo_source):
	...
	for i in range(len(tab_histo)):
		...
		histo_tempo_nor = [histo_tempo_nor[:256],histo_tempo_nor[256:512],histo_tempo_nor[512:]]
		...
		for k in range(len(histo_source[0])): # pixels are in queueleuleu in histo (list)
			...
			difference_s += abs(histo_tempo_nor[1][k] - histo_source_nor[1][k])**2
			difference_v += abs(histo_tempo_nor[2][k] - histo_source_nor[2][k])**2
...
def Moyenne_Image_Source(im_source_RGB,im_source_HSV,new_size,tab_histo,tab,tab_non_convert,pix_copy):
